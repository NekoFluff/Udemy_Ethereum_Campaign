...
describe('Campaigns', () => {
  ...
  it('allows people to contribute money and marks them as approvers', async () => {
    ...
    const isContributor2 = await campaign.methods.contributors(accounts[2]).call();
  ...
  it('requires minimum contribution', async () => {
    try {
      ...
      assert.equal(err.actual, undefined, 'Contract does not prevent user from contributing less than the required amount.');
    }
  })
  ...
  it('processes requests successfully', async () => {
    ...
    console.log('Difference in balance for account#2: ' + (balanceAfter - balancePrior));
  })
});